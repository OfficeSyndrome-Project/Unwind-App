...
class ScreeningStoredService {
  ...
  static Future<void> writeScreeningAnswer(
    ...
    await GeneralStoredService.writeBoolean(
        pagename, questionPage, questionId, value);
  }

  static Future<bool?> readScreeningAnswer(
      int questionPage, int questionId) async {
    var result = await GeneralStoredService.readBoolean(
        pagename, questionPage, questionId);
    return result;
  }

  static Future<void> removeScreeningAnswer(
      int questionPage, int questionId) async {
    var result =
        await GeneralStoredService.remove(pagename, questionPage, questionId);
    return result;
  }
}
